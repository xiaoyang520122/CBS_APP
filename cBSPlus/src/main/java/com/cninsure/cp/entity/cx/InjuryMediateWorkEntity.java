...
public class InjuryMediateWorkEntity extends CxWorkAddressBaseEntity {
    public String accidentTime;//	事故时间
    public String accidentAddress;//	事故地点
    public String partyOneName;//	甲(受害方)
    public Integer poSex = -1;//	甲(受害方)性别
    ...
    public Integer poNature = -1;//	户籍性质  0农业‘1非农业’2其他

    public String partyBName;//	乙(标的方)
    public Integer pbSex = -1;//	乙(标的方)性别
    ...
    public Set<Integer> medicalFee;//	医疗费用赔偿 0、医疗费 1、诊疗费 2、住院费 3、整容费 4、住院伙食补助费 5、后续治疗费 6、营养费
    public Set<Integer> casualtiesFee;//	死亡伤残赔偿  0、丧葬费 1、死亡补偿费 2、残疾赔偿金 3、交通费 4、护理费 5、康复费 6、残疾辅助器具费 7、住宿费 8、误工费 9、被抚养人生活费 10、受害人亲属办理丧葬事宜支出的交通费用

    public String[] getMedicalFee() {
        if (medicalFee == null) return new String[]{};
        String[] mdfStrArr = new String[medicalFee.size()];
        int i = 0;
        for (Integer item : medicalFee) {
            mdfStrArr[i++] = item + "";
        }
        return mdfStrArr;
    }

    public String[] getCasualtiesFee() {
        if (casualtiesFee==null) return new String[]{};
        String[] mdfStrArr = new String[casualtiesFee.size()];
        int i=0;
        for (Integer item:casualtiesFee) {
            mdfStrArr[i++] = item + "";
        }
        return mdfStrArr;
    }

    public Set<MedicalPaid> medicalPaid;
    public Set<MedicalPaid> casualtiesPaid;
    ...
    public Integer deliveryMode = -1;//	快递方式
    public Integer deliveryCompany = -1;//	快递公司
    ...
    public String getPoNature() {
        return poNature==null?"":poNature+"";
    }
    /**因后台需要字典值value传String类型，这里做转化*/
    public String getPoSex() {
        return poSex==null?"":poSex+"";
    }
    /**因后台需要字典值value传String类型，这里做转化*/
    public String getPbSex() {
        return pbSex==null?"":pbSex+"";
    }
}
