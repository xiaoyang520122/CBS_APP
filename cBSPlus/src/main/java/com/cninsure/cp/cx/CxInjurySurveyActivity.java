...
import com.cninsure.cp.cx.fragment.CxInjSurveyFragment;
...
import com.cninsure.cp.entity.cx.injurysurvey.InjurySurveyTaskData;
 ...
 * usefuLness: CBS_APP
 */
public class CxInjurySurveyActivity extends BaseActivity implements View.OnClickListener {


    public CxInjurySurveyTaskEntity taskEntity; //物损任务信息
    ...
    private CxInjSurveyFragment fg1;
...
//        ViewUtils.inject(this);
    ...
    private void dowloadDictType() {
        ...
        params.add("type");
        params.add("investigation_small_type,survey_result"); // 医疗费用赔偿MedicalFee ,死亡伤残赔偿casualtiesFee ,户籍性质poNature
...
//            cxWorkEntity = new CxSurveyWorkEntity();
    ...
    private void SaveWorkInfo(int status) {
        ...
        InjurySurveyWorkEntity damageEnt = taskEntity.data.contentJson;
...
//                workhelp = new CxWorkhelp(this, uploadView, cxSurveyDict.getDictByType("cxOrderWorkImageType"), null);
    ...
    private void getTaskInfo(String value) {
        try {
            taskEntity = JSON.parseObject(value, CxInjurySurveyTaskEntity.class);
        } catch (Exception e) {  //解析失败，关闭界面
            ErrorDialogUtil.showErrorAndFinish(this,"获取任务信息失败，请联系管理员！");
            e.printStackTrace();
        }
        if (taskEntity == null )  taskEntity = new CxInjurySurveyTaskEntity();
        if (taskEntity.data == null  ) taskEntity.data = new InjurySurveyTaskData();
        if (taskEntity.data.contentJson == null) taskEntity.data.contentJson = new InjurySurveyWorkEntity();
        initView();
    }
        ...
        if (baseEntity.success) {
            ...
            dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
                ...
                public void onDismiss(DialogInterface dialog) {
                    if (isSubmit==1)
                        CxInjurySurveyActivity.this.finish();
    ...
    /**处理签字图片*/
    private void signMeath(String url){  //没有签字功能
        if (url!=null) {
            if (fg0.signFlag==0){
