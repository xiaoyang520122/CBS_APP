...
public class PublicPhotoChoiceActivity extends Activity {
	...
	public static NotifyCallBack callBack; //接口回调刷新ExpanderbleListView
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initView();
		temPhotoEntitiys = new ArrayList<>();
...
//				PublicPhotoChoiceActivity.this.startActivity(intent);
				ImageDisplayUtil.displayByMyView(PublicPhotoChoiceActivity.this, fileNames.get(i));
			}
		});
	}

	private void initData() {
		MyMap myMap  = (MyMap) getIntent().getSerializableExtra("classImgMap");
		classImgMap = myMap.myMap;
	...
	private void initView() {
		...
		cancelTv.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				PublicPhotoChoiceActivity.this.finish();
			}
		});
		...
		sureTv.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				for (WorkPhotoEntitiy temEn:temPhotoEntitiys){
					displayPhoto(temEn.location);
				}
				PublicPhotoChoiceActivity.this.finish();
			}
		});
	...
	private void displayPhoto(final String jpgFilePath) {
		CxImagEntity imgEn = new CxImagEntity();
		...
		callBack.notifyDo(getIntent().getIntExtra("GroupId",0),imgEn);
	}
	...
	private void handleAndSaveImag(int postion) {
		...
		ImageUtil.compressBmp(PublicPhotoChoiceActivity.this, data, PicturePath);// 压缩、水印、储存
	...
	private class MyGridAdapter extends BaseAdapter {
		...
		public View getView(final int itemPostion, View conview, ViewGroup arg2) {
			...
			if (conview != null) {
				...
				vh = new ViewHolder();
				conview = LayoutInflater.from(PublicPhotoChoiceActivity.this).inflate(R.layout.gride_image_view, null);
			...
			Glide.with(PublicPhotoChoiceActivity.this).load(imgPath).into(vh.img);
