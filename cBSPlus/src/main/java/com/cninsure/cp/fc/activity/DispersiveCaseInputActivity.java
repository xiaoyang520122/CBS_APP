...
public class DispersiveCaseInputActivity extends BaseActivity implements OnClickListener, OnCheckedChangeListener, OnPageChangeListener {

	/***
	 * risk_type 险种大类
rev_case_type 接案方式
feiche_baoxian_type_other 创新及分散型险种
...
deputeItem 水险委托事项
	 ...
	 */
	String[] ZDparamsStr=new String[]{"risk_type","rev_case_type","feiche_baoxian_type_other","is_yd","depute_item",
			"business_type","case_industry","depute_sf","urgency_level","danger_res","loss_currency"};
	
	/**需要设置onclick事件的ID集合*/
	private int[] onclickIDs=new int[]{R.id.CaseINPUT_text2_D,R.id.CaseINPUT_text4_D,R.id.CaseINPUT_text6_D};
...
//	private List<CaseDepute> depsData;
	private LinearLayout ggslLayout;//relationalLayout,
	/**委托人信息*/
...
//				workhelp.upload();
	...
	private void initView() {
		choiceGGShelp=new ChoiceGGShelp(null,this,1);
...
//		spinnerView1=new ArrayList<Spinner>(11);
		creatCaseView=inflater.inflate(R.layout.fc_take_case_creatinfo_d, null);
		wtView=inflater.inflate(R.layout.fc_take_case_wt_info_d, null);
		chuxianView=inflater.inflate(R.layout.fc_take_case_event_info_d, null);
		otherView=inflater.inflate(R.layout.fc_take_case_other_info_d, null);
//		relationalLayout=(LinearLayout) otherView.findViewById(R.id.CaseINPUT_relationalLear);
		ggslLayout=(LinearLayout) otherView.findViewById(R.id.CaseINPUT_ggsLear_D);
//		depsData=new ArrayList<CaseDepute>();
//		otherView.findViewById(R.id.CaseINPUT_relationalAdd).setOnClickListener(this);
		otherView.findViewById(R.id.CaseINPUT_GGSAdd_D).setOnClickListener(this);
		/**设置邮件输入格式,错误是提示用户*/
		PatternUtil.setEmailInput((EditText)wtView.findViewById(R.id.CaseINPUT_edit7_D));
		
		search = (AutoCompleteTextView) wtView.findViewById(R.id.CaseINPUT_text5_D);  
//		((AutoCompleteTextView)wtView.findViewById(R.id.CaseINPUT_text5_D)).setError("委托人必须为从自动弹出的下拉列表中选择，否则录入可能会失败！");//设置委托人录入的规则
...
//		APPResponseModel<String> response=JSON.parseObject(value, APPResponseModel.class);
		...
		dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
			...
			public void onDismiss(DialogInterface arg0) {
				if (code.equals("0")) {
					CopyUtils.copy(DispersiveCaseInputActivity.this, caseNober);
					ToastUtil.showToastShort(DispersiveCaseInputActivity.this, "案件编号已复制到剪贴板");
					DispersiveCaseInputActivity.this.finish();
				}
			}
		});
		dialog.show();
...
//		(this, R.layout.spinner_item, paramsList));
	...
	private void getYYBInfo(String value) {
		...
		((Spinner)findViewById(R.id.CaseINPUT_SPINNYYB_D)).setAdapter(new ArrayAdapter<String>
...
//      search.setAdapter(adapter);
      ...
      choiceWTRhelp=new ChoiceWTRhelp(this, wtArr,search,wtRenData.data.list,wtView,
    		  R.id.CaseINPUT_edit5_D,R.id.CaseINPUT_edit6_D,R.id.CaseINPUT_edit7_D);
...
//		CaseManage casem=appRequestData.requestData.casem;
		...
		if (caseBeanEntity!=null && caseBeanEntity.data!=null && caseBeanEntity.data.casem!=null) {
			appRequestData.requestData.rels=caseBeanEntity.data.rels;
...
//		}
		...
		((EditText)creatCaseView.findViewById(R.id.CaseINPUT_edit2_D)).setText(casem.reciverPhone);//接案人联系方式
		
		for (int i = 0; i < YYBDataList.size(); i++) { //归属机构
			if (casem.gsOrg.equals(YYBDataList.get(i).id+"")) {
				((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINNYYB_D)).setSelection(i+1);
				break;
			}
		}
		((EditText)creatCaseView.findViewById(R.id.CaseINPUT_edit1_D)).setText(casem.caseName);//案件名称
		
		
		for (int i = 0; i < dictEntity.data.is_yd.size(); i++) {
			if (casem.isYd.equals(dictEntity.data.is_yd.get(i).value+"")) { //是否异地
				((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN2_D)).setSelection(i);//是否异地
			}
		}
		((TextView)creatCaseView.findViewById(R.id.CaseINPUT_text2_D)).setText(casem.deputeDate);//委托日期
...
//			}
//		}
		if (casem.deputeItem!=null ) { //回显委托内容
			String [] deputeItemArr= casem.deputeItem.split(",");
			getdeputeItems();wtTempstr="";
			for (int i = 0; i < deputeItemArr.length; i++) {
				for (publicData pData:dictEntity.data.depute_item) {
					if (deputeItemArr[i].equals(pData.value+"")) {
						wtTempstr = wtTempstr+pData.getLabel()+",";
						break;
					}
				}
				((TextView)wtView.findViewById(R.id.CaseINPUT_SPINN11_D)).setText(wtTempstr);
			}
		}
		
		for (int i = 0; i < dictEntity.data.feiche_baoxian_type_other.size(); i++) {
			if (casem.feicheBaoxianType.equals(dictEntity.data.feiche_baoxian_type_other.get(i).value+"")) {
				((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN4_D)).setSelection(i);//险种
...
//				List<String>  strings=caseTypeList;
				for (int j = 0; j < caseTypeList.size(); j++) {
					if (casem.caseType!=null && (casem.caseType+"").equals(map.get(caseTypeList.get(j)))) {
						((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN5_D)).setSelection(j);//险种分类
					}
				}
				break;
			}
...
//		}
		...
		for (int i = 0; i < dictEntity.data.case_industry.size(); i++) {
			if (casem.caseIndustry.equals(dictEntity.data.case_industry.get(i).value+"")) {
				((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN8_D)).setSelection(i); //所属行业
			}
		}

		((TextView)creatCaseView.findViewById(R.id.CaseINPUT_text4_D)).setText(casem.revCaseDate); //接案日期
		((EditText)creatCaseView.findViewById(R.id.CaseINPUT_edit3_D)).setText(casem.checkAdd);//查勘地点
		((EditText)creatCaseView.findViewById(R.id.CaseINPUT_edit4_D)).setText(casem.casePick); //报案摘要(2000字)
		
		/**委托信息***/
//		String wtName=((AutoCompleteTextView)wtView.findViewById(R.id.CaseINPUT_text5_D)).getText().toString(); //委托人
		...
		if (wtRenData!=null && wtRenData.data!=null && wtRenData.data.list!=null) {
			for (int i = 0; i < wtRenData.data.list.size(); i++) {
				if (casem.deputeId.equals(wtRenData.data.list.get(i).id+"")) {
					((AutoCompleteTextView)wtView.findViewById(R.id.CaseINPUT_text5_D)).setText(wtRenData.data.list.get(i).name);//委托人
					break;
				}
			}
		}
		((EditText)wtView.findViewById(R.id.CaseINPUT_edit5_D)).setText(casem.deputeLinkPer); //委托人联系人
		((EditText)wtView.findViewById(R.id.CaseINPUT_edit6_D)).setText(casem.deputeLinkTel); //委托人联系人电话
		((EditText)wtView.findViewById(R.id.CaseINPUT_edit7_D)).setText(casem.deputeLinkEmail); //委托人联系人Email
...
//		}
		...
		((EditText)wtView.findViewById(R.id.CaseINPUT_edit12_D)).setText(casem.wtfYq); //委托方要求
...
//				.getSelectedItemPosition()).value+"";//委托事项
		/**出险信息***/
		((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit13_D)).setText(casem.insurerCaseNo); //保险公司报案号
...
//		((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit16)).setText(casem.remark); //出险单位
		((TextView)chuxianView.findViewById(R.id.CaseINPUT_text6_D)).setText(casem.dangerPeriod); //出险日期
		((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit17_D)).setText(casem.dangerAdd);//出险地点
		for (int i = 0; i < dictEntity.data.danger_res.size(); i++) {
			if (casem.dangerRes.equals(dictEntity.data.danger_res.get(i).value+"")) {
				((Spinner)chuxianView.findViewById(R.id.CaseINPUT_SPINNres_D)).setSelection(i);//出险原因
			}
		}
		((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit19_D)).setText(casem.lossAmout); //报损/索赔金额
		
		for (int i = 0; i < dictEntity.data.loss_currency.size(); i++) {
			if (casem.lossCurrency.equals(dictEntity.data.loss_currency.get(i).value+"")) {
				((Spinner)chuxianView.findViewById(R.id.CaseINPUT_SPINN12_D)).setSelection(i);//报损币别
			}
		}
		
		((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit20_D)).setText(casem.cxUintLink); //出险单位联系人
		((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit21_D)).setText(casem.lxPhone); //联系人电话
		((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit22_D)).setText(casem.emial); //邮箱
		((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit23_D)).setText(casem.cxJg); //出险经过
		((EditText)otherView.findViewById(R.id.CaseINPUT_edit24_D)).setText(casem.remarks); //备注(2000字)：
...
//		setonXZclick((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN1));
		/**是否异地：：**/
		((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN2_D)).setAdapter(new ArrayAdapter<String>
...
//		(this, R.layout.spinner_item, dictEntity.getDictArr(dictEntity.data.rev_case_type)));
		/**险种**/
		XZSpinner=((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN4_D));
		XZSpinner.setAdapter(new ArrayAdapter<String>
		(this, R.layout.spinner_item, dictEntity.getDictArr(dictEntity.data.feiche_baoxian_type_other)));
		setXZSpinneronlick();
		/**业务类型：**/
...
//		
		/**所属行业：**/
		((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN8_D)).setAdapter(new ArrayAdapter<String>
...
//		(this, R.layout.spinner_item, dictEntity.getDictArr(dictEntity.data.urgency_level )));
		/**委托事项： **/
		((TextView)wtView.findViewById(R.id.CaseINPUT_SPINN11_D)).setOnClickListener(this);
//		setAdapter(new ArrayAdapter<String>(this, R.layout.spinner_item, dictEntity.getDictArr(dictEntity.data.depute_item )));
		/** 报损币别：**/
		((Spinner)chuxianView.findViewById(R.id.CaseINPUT_SPINN12_D)).setAdapter(new ArrayAdapter<String>
		(this, R.layout.spinner_item, dictEntity.getDictArr(dictEntity.data.loss_currency )));
		/**出险原因**/
		((Spinner)chuxianView.findViewById(R.id.CaseINPUT_SPINNres_D)).setAdapter(new ArrayAdapter<String>
...
//				if (arg2==1) {
//					DispersiveCaseInputActivity.this.startActivity(
//							new Intent(DispersiveCaseInputActivity.this, WaterCaseInputActivity.class));
//					DispersiveCaseInputActivity.this.finish();
//				}
//			}
//			@Override
...
//	}
	...
	private void setXZSpinneronlick() {
		XZSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				/**险种分类：**/
				caseTypeList=FCCaseTypeEntity.getTypeList(dictEntity.data.feiche_baoxian_type_other.get(arg2).value);
				((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN5_D)).setAdapter(new ArrayAdapter<String>
				(DispersiveCaseInputActivity.this, R.layout.spinner_item, caseTypeList));
			}
			@Override
			public void onNothingSelected(AdapterView<?> arg0) { }
		});
...
//			addRelationals();
//			break;
		case R.id.CaseINPUT_GGSAdd_D://添加作业人员信息
			choiceGGShelp.showChoiceDialog();
			break;
		case R.id.CaseINPUT_text5_D://选择委托人
			choiceWTRhelp.showChoiceDialog();
			break;
		case R.id.CaseINPUT_SPINN11_D://选择委托事项
			showChoiceWindow();
			break;
	...
	String wtTempstr = "",wtidTempstr = "",wtTempstr1 = "",wtidTempstr1 = "";
	 /**获取委托事项*/
	private void getdeputeItems(){
		String [] deputeItemArr=getDeputeItems();
		if (dictEntity.data.depute_item.size()>0) {
			...
			for (int i = 0; i < wtinfos.length; i++) {
				...
				for (String str:deputeItemArr) {
					if (str.equals(dictEntity.data.depute_item.get(i).value+"")) {
						checkedItems[i]=true;
						break;
					}
				}
	...
	/**获取后台返回的已选择委托事项**/
	private String[] getDeputeItems(){
		String [] deputeItemArr;
		if (appRequestData.requestData.casem!=null && appRequestData.requestData.casem.deputeItem!=null) {
			deputeItemArr= appRequestData.requestData.casem.deputeItem.split(",");
		}else {
			deputeItemArr=new String []{};
		}
		return deputeItemArr;
...
//					if (wtTempstr1) { //委托事项点击查看后直接退出会报错！
//						
//					}
...
//////				.getSelectedItemPosition()).value;//险种大类
		...
		casem.caseName=((EditText)creatCaseView.findViewById(R.id.CaseINPUT_edit1_D)).getText().toString();//案件名称
		
		casem.isYd=dictEntity.data.is_yd.get(((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN2_D))
				.getSelectedItemPosition()).value+"";//是否异地
		
		casem.deputeDate=((TextView)creatCaseView.findViewById(R.id.CaseINPUT_text2_D)).getText().toString();//委托日期
...
//				.getSelectedItemPosition()).value+"";//接案方式
		...
		casem.feicheBaoxianType=dictEntity.data.feiche_baoxian_type_other.get(((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN4_D))
				.getSelectedItemPosition()).value+"";//险种
		
		String caseTypeStr=caseTypeList.get((((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINN5_D)).getSelectedItemPosition()));
...
//		int DzxxPostion=((Spinner)creatCaseView.findViewById(R.id.CaseINPUT_SPINNYYB_D)).getSelectedItemPosition();
//		if (DzxxPostion>0) {
...
//		}
		...
		casem.checkAdd=((EditText)creatCaseView.findViewById(R.id.CaseINPUT_edit3_D)).getText().toString(); //查勘地点
		casem.casePick=((EditText)creatCaseView.findViewById(R.id.CaseINPUT_edit4_D)).getText().toString(); //报案摘要(2000字)
		...
		casem.deputeLinkPer=((EditText)wtView.findViewById(R.id.CaseINPUT_edit5_D)).getText().toString(); //委托人联系人
		casem.deputeLinkTel=((EditText)wtView.findViewById(R.id.CaseINPUT_edit6_D)).getText().toString(); //委托人联系人电话
		casem.deputeLinkEmail=((EditText)wtView.findViewById(R.id.CaseINPUT_edit7_D)).getText().toString(); //委托人联系人Email
...
//				.getSelectedItemPosition()).value+""; //紧急程度
		casem.wtfYq=((EditText)wtView.findViewById(R.id.CaseINPUT_edit12_D)).getText().toString(); //委托方要求
//		casem.deputeItem=dictEntity.data..get(((Spinner)wtView.findViewById(R.id.CaseINPUT_SPINN10))
//				.getSelectedItemPosition()).value+"";//委托事项
		/**出险信息***/
		casem.insurerCaseNo=((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit13_D)).getText().toString(); //保险公司报案号
...
//		casem.remark=((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit16)).getText().toString(); //出险单位
		casem.dangerPeriod=((TextView)chuxianView.findViewById(R.id.CaseINPUT_text6_D)).getText().toString(); //出险日期
		casem.dangerAdd=((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit17_D)).getText().toString();//出险地点
		casem.dangerRes=dictEntity.data.danger_res.get(((Spinner)chuxianView.findViewById(R.id.CaseINPUT_SPINNres_D))
				.getSelectedItemPosition()).value+"";//出险原因
		casem.lossAmout=((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit19_D)).getText().toString(); //报损/索赔金额
		casem.lossCurrency=dictEntity.data.loss_currency.get(((Spinner)chuxianView.findViewById(R.id.CaseINPUT_SPINN12_D))
				.getSelectedItemPosition()).value+"";//报损币别
		casem.cxUintLink=((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit20_D)).getText().toString(); //出险单位联系人
		casem.lxPhone=((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit21_D)).getText().toString(); //联系人电话
		casem.emial=((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit22_D)).getText().toString(); //邮箱
		casem.cxJg=((EditText)chuxianView.findViewById(R.id.CaseINPUT_edit23_D)).getText().toString(); //出险经过
		casem.remarks=((EditText)otherView.findViewById(R.id.CaseINPUT_edit24_D)).getText().toString(); //备注(2000字)：
...
//	}
	...
	private void setonclickandValue(final TextView textTv) {
		...
		textTv.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View arg0) {
				DatePickerDialog pickerDialog=new DatePickerDialog(DispersiveCaseInputActivity.this, AlertDialog.THEME_DEVICE_DEFAULT_LIGHT
						, new OnDateSetListener() {
							@Override
							public void onDateSet(DatePicker arg0, int y, int m, int d) {
								String checkDate=y+"-"+getM(m+1)+"-"+getD(d);
		...
		spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				if (spinner.getTag()==null) {
					...
					ToastUtil.showToastShort(DispersiveCaseInputActivity.this, "选择公估师"+appRequestData.requestData.rels.get(i).userName+"为主办");
					castGGS(i);
				}
