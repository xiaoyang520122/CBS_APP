...
//            ActivityCollector.finishAll();//有时候异常抛出后重启会进入之前的activity，这样部分数据报空，如用户信息，为了避免直接全部杀死，然后重新登录。
    ...
    private String writeFile(String sb) throws Exception {  
        ...
        String fileName = "crash-" + AppApplication.sp.getString("CrashUserName", "无用户名") +"·"+time + ".txt";  
...
//        }  
    ...
    private void saveToHttp(String sb) {
    	List<NameValuePair> params=new ArrayList<NameValuePair>();
    	params.add(new BasicNameValuePair("userId", userId));
    	params.add(new BasicNameValuePair("errorLog", "CBS用户名："+AppApplication.sp.getString("CrashUserName", "无用户名")+"="+sb));
    	params.add(new BasicNameValuePair("versionCode", getVersion()));
    	params.add(new BasicNameValuePair("type", "1"));
    	params.add(new BasicNameValuePair("createTime", formatter.format(new Date())));
    	requestPost("http://118.89.40.53:8080/new_cbs/app/interface/errorlog/save", params, 0);
	}
