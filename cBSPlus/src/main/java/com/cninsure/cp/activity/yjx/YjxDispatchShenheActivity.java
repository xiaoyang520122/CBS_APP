...
import com.cninsure.cp.entity.yjx.YjxShenheMsgEntity;
import com.cninsure.cp.entity.yjx.YjxShenheMsgEntity.SHHTableData.SHHData.SHHListEntity;
import com.cninsure.cp.entity.yjx.errorTypeUtil;
...
public class YjxDispatchShenheActivity extends BaseActivity implements OnCheckedChangeListener, OnClickListener, OnPageChangeListener {
	private View wtInfoView, workView, shenheView; // 三个现象卡内容View
	...
	/** 上传的委托资料 */
	private LinearLayout wtFileLinear, workFileLinear,errorLayout;
	...
	/**审核信息**/
	private YjxShenheMsgEntity SHHMsg;
	/**错误类型*/
	private ErrorTypeEntity errorTypeEn;
	/**编辑的差错类型*/
	private List<SHHListEntity> listCommands;
	...
	protected void onResume() {
		if ("seeWorkInfo".equals(getIntent().getStringExtra("requestType"))) {
			...
			shenheView.findViewById(R.id.YjxSHV_shenhelayout).setVisibility(View.GONE);
			shenheView.findViewById(R.id.YjxSHV_shenhechaozuoTv).setVisibility(View.GONE);
		}
		super.onResume();
	}
	...
	private void initView() {
		...
		findViewById(R.id.ACYJXWORK_submitLinearlayout).setVisibility(View.GONE);
		findViewById(R.id.ACYJXWORK_btn_00).setVisibility(View.VISIBLE);
		...
		shenheView = inflater.inflate(R.layout.yjx_shenhe_view, null);

		ViewDisableUtil.disableSubControls((ScrollView) wtInfoView);
		ViewDisableUtil.disableSubControls((ScrollView) workView);

		shenheView.findViewById(R.id.YjxSHV_backButton).setOnClickListener(this);
		shenheView.findViewById(R.id.YjxSHV_passButton).setOnClickListener(this);
		shenheView.findViewById(R.id.YjxSHV_add).setOnClickListener(this);
		...
		errorLayout = (LinearLayout) shenheView.findViewById(R.id.YjxSHV_errorMsgLayout);
		viewlist.add(shenheView);
		viewlist.add(wtInfoView);
		viewlist.add(riskersView);
		viewlist.add(workView);
		initviewpageradapter();
		vpager.setAdapter(pagerAdapter);
		radgrup.check(R.id.ACYJXWORK_btn_00);
	...
	private void workInfoViewInit() {
		...
		workView.findViewById(R.id.YJXWV_chocieByAlbum).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				PickPhotoUtil.albumPhoto(YjxDispatchShenheActivity.this, PickPhotoUtil.PHOTO_REQUEST_ALBUMPHOTO_W);
			}
		});
	...
	public void onClick(View arg0) {
		...
		case R.id.ACYJXWORK_submitButton:
			...
			submitChoiceFile();
			break;
		case R.id.YjxSHV_backButton: // 任务审核退回
			orderReject();
			break;
		case R.id.YjxSHV_passButton: // 任务审核通过
			orderSHPass();
			break;
		case R.id.YjxSHV_add: // 添加差错信息
			addError();
			break; 
	...
	private void addError() {
		if (listCommands==null) {
			listCommands=new ArrayList<YjxShenheMsgEntity.SHHTableData.SHHData.SHHListEntity>();
		}
		//请求
		HttpUtils.requestGet("http://dict.cnsurvey.cn:7077/cninsure-disc-ws/dicts/list?type=errorType", new ArrayList<String>()
				, HttpRequestTool.GET_ERROR_TYPE);
//		userId:User-20190307155832-17A2AEB4
//		client:192.168.1.1
//		timestamp:1566979734308
//		myToken:20190828160856503-53960-5D542
//		orderUid:YJX-DISPATCH-20190415100607-493D5
//		id:51
//		surveyDescription:大连房间
//		surveyConclusion:测试调查结论
//		analysis:阿萨德
//		conclusion:是大法官
//		auditMessage:是大法官
//		listCommands[0].errorType:39
//		listCommands[0].errorMessage:类型不全面
//		listCommands[0].errorPoints:7
//		digest:9cb2837298cfa03061243172cf07b76f
	}
	...
	private void orderReject() {
		...
		params.add(new BasicNameValuePair("userId", AppApplication.getUSER().data.userId + ""));
		 ...
		 params.add(new BasicNameValuePair("surveyConclusion", workDataEn.surveyConclusion));
		 String analysis = ((EditText)shenheView.findViewById(R.id.YjxSHV_analysis)).getText().toString();
		 params.add(new BasicNameValuePair("analysis", analysis));
		 String conclusion = ((EditText)shenheView.findViewById(R.id.YjxSHV_analysis)).getText().toString();
		 if (TextUtils.isEmpty(conclusion)) {
			DialogUtil.getErrDialog(this, "审核退回请填写“审核结论”（退回原因）!").show();
			return;
		}
		 params.add(new BasicNameValuePair("conclusion", conclusion));
		 params.add(new BasicNameValuePair("auditMessage", conclusion));
	...
	private void orderSHPass() {
		...
		params.add(new BasicNameValuePair("userId", AppApplication.getUSER().data.userId + ""));
		 ...
		 if (TextUtils.isEmpty(analysis)) {
				DialogUtil.getErrDialog(this, "请填写“审核分析”!").show();
				return;
			}
		 params.add(new BasicNameValuePair("analysis", analysis));
		 String conclusion = ((EditText)shenheView.findViewById(R.id.YjxSHV_conclusion)).getText().toString();
		 if (TextUtils.isEmpty(conclusion)) {
			DialogUtil.getErrDialog(this, "请填写“审核结论”!").show();
			return;
		}
		 if (listCommands!=null) {
			for (int i = 0; i <listCommands.size(); i++) {
				params.add(new BasicNameValuePair("listCommands["+i+"].errorType", listCommands.get(i).errorType));
				params.add(new BasicNameValuePair("listCommands["+i+"].errorMessage", listCommands.get(i).errorMessage));
				params.add(new BasicNameValuePair("listCommands["+i+"].errorPoints", listCommands.get(i).errorPoints));
			}
		}
		 params.add(new BasicNameValuePair("conclusion", conclusion));
		 params.add(new BasicNameValuePair("auditMessage", conclusion));
		HttpUtils.requestPost(URLs.YJX_SHENHE_PASS, params, HttpRequestTool.YJX_SHENHE_PASS);
	}
	...
	public void submitWorkInfo() {
		final List<NameValuePair> params = new ArrayList<NameValuePair>();
		// if (workDataEn==null) {
		// workDataEn = new YjxCaseWorkEntity();
		// }
		IsFull = true;
		// workDataEn.injuredList = injuredData;
		// workDataEn.dispatchId = dispatchEn.id;
		// workDataEn.dispatchUid = dispatchEn.uid;
		// workDataEn.surveyTime =
		// getDateInfo((TextView)workView.findViewById(R.id.YJXWV_lookTime),true,true);
		// workDataEn.surveyAddress =
		// getTextInfo((EditText)workView.findViewById(R.id.YJXWV_VisitingAddress),true,true);
		// workDataEn.surveyType =
		// getTextInfo((Spinner)workView.findViewById(R.id.YJXWV_lookMode),true,true,"未选择调查方式！");
		// workDataEn.surveyDescription =
		// getTextInfo((EditText)workView.findViewById(R.id.YJXWV_Investigation),true,true);
		// workDataEn.surveyConclusion =
		// getTextInfo((EditText)workView.findViewById(R.id.YJXWV_VisitResults),true,true);
		// workDataEn.fileUrls = fileUrlStrs;
		// workDataEn.surveyData = surveyDataTem;
		// workDataEn.status = 1;
		...
		for (int j = 0; j < injuredData.size(); j++) {
			if (injuredData == null || injuredData.get(j) == null || TextUtils.isEmpty(injuredData.get(j).diagnostic)
					|| TextUtils.isEmpty(injuredData.get(j).hospital) || TextUtils.isEmpty(injuredData.get(j).idCard)
					|| TextUtils.isEmpty(injuredData.get(j).name) || TextUtils.isEmpty(injuredData.get(j).sex)) {
		...
		try {
			for (int j = 0; j < injuredData.size(); j++) {
				...
				// params.add(new
				// BasicNameValuePair("injuredList["+j+"].caseId",injuredData.get(j).caseId+""));
				...
				// params.add(new
				// BasicNameValuePair("injuredList["+j+"].id",injuredData.get(j).id+""));
				...
				// params.add(new
				// BasicNameValuePair("injuredList["+j+"].workId",injuredData.get(j).workId+""));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void wtInfoViewInit() {
		...
		wtInfoView.findViewById(R.id.YJXWWV_chocieByAlbum).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				PickPhotoUtil.albumPhoto(YjxDispatchShenheActivity.this, PickPhotoUtil.PHOTO_REQUEST_ALBUMPHOTO);
			}
		});
	...
	public void setSmallTypeAdapter(List<NameValuePair> typeMap, int point) {
		...
		smallTypeValues.clear();
		smallTypeValues.addAll(InsuranceTypeUtil.MapToList(smalltypeMap));
		smallTypeAdatper = new ArrayAdapter<String>(YjxDispatchShenheActivity.this, android.R.layout.simple_list_item_1, smallTypeValues);
	...
	private void ListViewAddFooterView() {
		...
		// riskersView.setEmptyView(inflater.inflate(R.layout.yjx_injured_empty,
		// null)); //放出来后addFooterView不显示，无法添加伤者信息
	...
	/**
	 * 弹出窗口填写伤者信息，并在传递监听事件中完成对当前Activity中伤者信息的添加并显示 itemId>-1代表修改，==-1表示添加
	 */
	private void getAndDisplayInjured(final int itemId) {
		AddInjuredUtil addInjuredUtil = new AddInjuredUtil();
		if (itemId == -1) {
			addInjuredUtil.getInjured(YjxDispatchShenheActivity.this, null, getDismisListener(addInjuredUtil, itemId));
		} else {
			addInjuredUtil.getInjured(YjxDispatchShenheActivity.this, injuredData.get(itemId), getDismisListener(addInjuredUtil, itemId));
		}
	}
	...
	private void disPlayriskersView() {
		...
		ViewDisableUtil.disableSubControls((ListView) riskersView);
	}

	/** 获取调度任务的作业信息 */
	private void downLoadData() {
		LoadDialogUtil.setMessageAndShow(YjxDispatchShenheActivity.this, "加载中……");
		// 获取作业信息
	...
	public void eventData(List<NameValuePair> values) {
		...
		case HttpRequestTool.POST_YJX_BAOAN_UPDATE: // 医健险作业界面接报案保存成功
			...
			break;
		case HttpRequestTool.YJX_SHENHE_REJECT: // 审核退回
			rejectRequestInfo(values);
			break;
		case HttpRequestTool.YJX_SHENHE_PASS: // 审核退回
			passRequestInfo(values);
			break;
		case HttpRequestTool.GET_ORDER_STATUS: //审核信息
			showSHHMessage(values.get(0).getValue());
			break; 
		case HttpRequestTool.GET_ERROR_TYPE: //获取差错类型信息
			getErrorTypeMsg(values.get(0).getValue());
			break; 
	...
	/**获取错误信息类型*/
	private void getErrorTypeMsg(String value) {
		try {
			errorTypeEn = JSON.parseObject(value, ErrorTypeEntity.class);
			if (errorTypeEn!=null) {
				new YjxAddErrorHelper().addOrEditError(this, null,-1,errorTypeEn, new BcallSet() {
					@Override
					public void setValue(SHHListEntity eEn,int point) {
						if (point==-1) { //修改回调
							listCommands.add(eEn);
						}else {
							listCommands.set(point, eEn);
						}
						displayErrorMsg();
					}
				});
			}else {
				DialogUtil.getErrDialog(this, "获取错误类型失败！请重试。").show();
			}
		} catch (Exception e) {
			DialogUtil.getErrDialog(this, "获取错误类型失败！请重试。").show();
			e.printStackTrace();
	...
	private void displayErrorMsg() {
		...
		if (listCommands!=null) {
			 for ( int i = 0; i < listCommands.size(); i++) {
				 View view = inflater.inflate(R.layout.error_shenhe_display_view, null);
					((TextView) view.findViewById(R.id.YJXerrSHDV_label)).setText(errorTypeUtil.getErrorTypeMsgById(errorTypeEn, listCommands.get(i).errorType));
					((TextView) view.findViewById(R.id.YJXerrSHDV_errorMessage)).setText(listCommands.get(i).errorMessage);
					((TextView) view.findViewById(R.id.YJXerrSHDV_errorPoints)).setText(listCommands.get(i).errorPoints);
					//修改差错信息
					editErrorMsg(view, i);
					//删除差错信息
					deleteErrorInfo(view, i);
					errorLayout.addView(view);
			}
		}
	}
	
	/**修改差错信息*/
	private void editErrorMsg(View view,final int i){
		view.findViewById(R.id.YJXerrSHDV_edit).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
				new YjxAddErrorHelper().addOrEditError(YjxDispatchShenheActivity.this, listCommands.get(i),i,errorTypeEn, new BcallSet() {
					@Override
					public void setValue(SHHListEntity eEn,int point) {
						if (point==-1) { //修改回调
							listCommands.add(eEn);
	...
	/**删除差错信息*/
	private void deleteErrorInfo(View view,final int i){
		//删除差错信息
		view.findViewById(R.id.YJXerrSHDV_delete).setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View arg0) {
	...
	private void showSHHMessage(String value) {
		SHHMsg= JSON.parseObject(value, YjxShenheMsgEntity.class);
		StringBuffer SHMsg = new StringBuffer();
		...
		if (SHHMsg!=null && SHHMsg.tableData != null && SHHMsg.tableData.data != null && SHHMsg.tableData.data.get(0).createDate!=null) {
			for (int i = 0; i < SHHMsg.tableData.data.size(); i++) {
				...
				SHMsg.append("审核" + (i+1) + "（" + ispass + "）\n");
				SHMsg.append("审核时间："+SHHMsg.tableData.data.get(i).createDate+"\n");
				SHMsg.append("调查经过："+SHHMsg.tableData.data.get(i).surveyDescription+"\n");
				SHMsg.append("调查结论："+SHHMsg.tableData.data.get(i).surveyConclusion+"\n");
				SHMsg.append("审核分析："+SHHMsg.tableData.data.get(i).analysis+"\n");
				SHMsg.append("审核结论："+SHHMsg.tableData.data.get(i).conclusion+"\n");
				SHMsg.append("审核分析："+SHHMsg.tableData.data.get(i).createDate+"\n");
				String cctype="";
				...
				SHMsg.append("差错信息："+cctype+"\n\n");
			}
		}else {
			SHMsg.append("");
		}
		((TextView)shenheView.findViewById(R.id.YjxSHV_shenheHistory)).setText(SHMsg.toString());
	...
	/**审核通过是否成功*/
	private void passRequestInfo(List<NameValuePair> values) {
		if (values!=null && values.size()>0 && !TextUtils.isEmpty(values.get(0).getValue())) {
			if (values.get(0).getValue().indexOf("成功")>-1) {
				DialogUtil.getAlertOneButton(this, values.get(0).getValue(), new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface arg0, int arg1) {
						YjxDispatchShenheActivity.this.finish();
					}
				}).show();
			}else {
				DialogUtil.getErrDialog(this, "审核失败："+values.get(0).getValue()).show();
			}
		}else {
			DialogUtil.getErrDialog(this, "审核失败！！").show();
		}
	}
	 ...
	 * @param values
	 */
	private void rejectRequestInfo(List<NameValuePair> values) {
		if (values!=null && values.size()>0 && !TextUtils.isEmpty(values.get(0).getValue())) {
			if (values.get(0).getValue().indexOf("成功")>-1) {
				DialogUtil.getAlertOneButton(this, values.get(0).getValue(), new DialogInterface.OnClickListener() {
					@Override
				...
				DialogUtil.getErrDialog(this, "退回失败："+values.get(0).getValue()).show();
			}
		}else {
			DialogUtil.getErrDialog(this, "退回失败！！").show();
		}
	}
	...
	private void showWorkSavaInfo(List<NameValuePair> values) {
		...
		if (responsecode == 200) {
			...
			// workDataEn = JSON.parseObject(values.get(0).getValue(),
			// YjxCaseWorkEntity.class);
		} else {
			LoadDialogUtil.dismissDialog();
			...
			for (int j = 0; j < fileArrs.length; j++) {
				...
				if (!TextUtils.isEmpty(fileName)) {
					...
					if ("image".equals(OpenFileUtil.getFileType(fileName))) { // 是图片就加在缩略图
						...
						Glide.with(this).load(OpenFileUtil.getTypeResouse(fileName, YjxDispatchShenheActivity.this)).centerCrop()
								.error(R.drawable.ssdk_weibo_empty_failed).into(imgV);
					}
		...
		view.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if ("image".equals(OpenFileUtil.getFileType(fileName))) { // 是图片就用图片工具加载
					ImageDisplayUtil.displayByMyView(YjxDispatchShenheActivity.this, fileName);
					...
					if (fileName.indexOf("://") > -1) {
						...
						// new
						// FileDownOpenUtil(YjxSurveyActivity.this).downloadAndOpen(fileName,
						// name);
					} else {
						OpenFileUtil.openFileByPath(YjxDispatchShenheActivity.this, fileName);
					}
				}
			}
		});
	...
	private void setWorkInfo() {
		...
		// 审核界面信息回显
		SetTextUtil.setTextViewText((TextView) shenheView.findViewById(R.id.YjxSHV_surveyDescription), workDataEn.surveyDescription);// 调查经过
		SetTextUtil.setTextViewText((TextView) shenheView.findViewById(R.id.YjxSHV_surveyConclusion), workDataEn.surveyConclusion);// 调查结论
	}
